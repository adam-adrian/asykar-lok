...
async function run() {
  const browser = spawn('helium-browser', [
    ...
    '--disable-gpu',
    '--incognito',
    '--user-data-dir=/tmp/helium-options-' + Date.now(),
    '--remote-debugging-port=9223',
    '--window-size=420,950',
    'http://127.0.0.1:8899/docs/mockup/match-card-options.html'
  ]);
  ...
  try {
    ...
    fs.writeFileSync(`${ARTIFACT_DIR}/match_card_structure_options.png`, Buffer.from(shot.data, 'base64'));
    console.log('Saved match_card_structure_options.png');
